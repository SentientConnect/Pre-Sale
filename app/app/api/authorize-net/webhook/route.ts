...
const PAID_TAG = 'SentientOS Preorder | $25 Paid'
...
function findCustomField(
  ...
  const target = normalizeFieldName(desiredName)
...
async function getAuthorizeTransaction(
  ...
  try {
    data = parseJson<AuthorizeTransactionResponse>(
      rawResponse,
    )
  } catch {
    console.error(
  ...
  const processorError = data.messages?.message
...
async function getGhlCustomFields(
  ...
  const data =
    parseJson<GhlCustomFieldsResponse>(rawResponse)
...
function buildCustomFieldValues(
  ...
  return values.flatMap(({ name, value }) => {
    const field = findCustomField(fields, name)
...
async function upsertGhlContact(args: {
  ...
  try {
    data = parseJson<GhlUpsertResponse>(rawResponse)
  } catch {
    throw new Error(
...
export async function POST(request: Request) {
  ...
  if (!signatureHeader) {
    return NextResponse.json(
      { error: 'Missing Authorize.net signature' },
      { status: 401 },
    )
  }
  ...
  try {
    webhook = parseJson<AuthorizeWebhook>(rawBody)
  } catch {
    return NextResponse.json(
  ...
  if (webhook.eventType !== EXPECTED_EVENT) {
    return NextResponse.json({
      ok: true,
      ignored: true,
  ...
  if (
    webhook.payload?.entityName !== 'transaction' ||
    !webhook.payload.id
  ) {
    return NextResponse.json(
  ...
  try {
    ...
    if (
      responseCode !== '1' ||
      !successfulStatuses.has(transactionStatus) ||
      Math.abs(amount - 25) > 0.001 ||
      !invoiceNumber.startsWith('SENT-')
    ) {
      console.warn(
    ...
    let ghlFields: GhlCustomField[] = []

    try {
      ghlFields = await getGhlCustomFields(
        ghlToken,
        ghlLocationId,
      )
    } catch (error) {
    ...
    const customFields = buildCustomFieldValues(
      ghlFields,
      [
        {
          name: 'Preorder Product',
          value: 'SentientOS™ AR Glasses',
        },
        {
          name: 'Preorder Deposit Amount',
          value: 25,
        },
        {
          name: 'Preorder Payment Status',
          value: 'Paid',
        },
        {
          name: 'Preorder Payment Provider',
          value: 'Authorize.net',
        },
        {
          name: 'Preorder Payment Date',
          value: paymentDate,
        },
        {
          name: 'Preorder Transaction ID',
          value: transactionId,
        },
        {
          name: 'Preorder Reference ID',
          value:
            webhook.payload.merchantReferenceId ||
            invoiceNumber,
        },
        {
          name: 'Preorder Source',
          value: 'presale.sentientconnect.io',
        },
      ],
    )
    ...
    const alreadyPaid = existingTags.some(
      (tag) =>
        tag.toLowerCase() ===
